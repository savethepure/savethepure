<?php
defined('BASEPATH') OR exit('No direct script access allowed');
?>
<?php include('static/helmet.php'); ?>

<?php include('static/header.php'); ?>

<title>Save The Pure | Registration Success</title>

<div class="wrapper-container py3"> 
    <div class="login-container col-10 md-col-10 sm-col-8 mx-auto py20">
        <h3 class="mt0 mb2" align="center">Registration Succeed</h3>
        <h5 align="center">your registration is succeed, in order to login, please check your email to verify your account.</h5>
    </div>
</div>
